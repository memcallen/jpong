...
import Main.SerialPortComm.PortReader;
...
public class Main extends Thread {
	...
	public void showSetup() {
		...
		start.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				boolean run = !Arrays.asList(comm.getAvailableSerialPorts()).contains(ArdP);
				boolean temp = true;
				try {
				...
				} else if (doArduino && !Arrays.asList(comm.getAvailableSerialPorts()).contains(ArdP)) {
					String commList = Arrays.asList(comm.getAvailableSerialPorts()).toString();

					commList = commList.replace('[', (char) 0).replace(']', (char) 0);

					JOptionPane.showMessageDialog(null,
							"There is no " + ardPort.getText() + " port available (Available ports:" + commList + ")");
				}
			}
		});
			...
			try {
				...
				Thread reader = new Thread(new Runnable() {
					...
					public void run() {
						...
						PortReader reader = comm.getReader();

						while (!Main.interrupted()) {
							one.location = reader.left;
							two.location = reader.right;
							Thread.yield();
						}
